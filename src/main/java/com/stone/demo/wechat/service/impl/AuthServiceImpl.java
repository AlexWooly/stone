...
import com.stone.demo.redis.RedisUtils;
import com.stone.demo.util.JsonData;
import com.stone.demo.wechat.constant.Constant;
import com.stone.demo.wechat.domain.User;
import com.stone.demo.wechat.jwt.JwtSecurityProperties;
import com.stone.demo.wechat.service.WxMiniApi;
import com.stone.demo.wechat.service.AuthService;
import com.stone.demo.wechat.service.UserService;
import com.stone.demo.wechat.service.dto.AuthUserDto;
import com.stone.demo.wechat.utils.JwtTokenUtils;
import com.stone.demo.wechat.utils.NetworkUtils;
import com.stone.demo.wechat.utils.WeChatUtil;
...
public class AuthServiceImpl implements AuthService {
    ...
    public JsonData login(AuthUserDto authUserDto, HttpServletRequest request) {
...
//        Result<AuthUserDto> result = new Result<>();
        ...
        if (!StringUtils.isEmpty(authUserDto.getAuthType()) && authUserDto.getAuthType() == 1) {
            ...
            if (StringUtils.isEmpty(openId)) {
                return JsonData.buildError(jsonObject.getString(Constant.ERR_CODE), jsonObject.getString(Constant.ERR_MSG));
            }
            authUserDto.setOpenId(openId);

            //判断用户表中是否存在该用户，不存在则进行解密得到用户信息，并进行新增用户
            JsonData resultUser = userService.findByOpenId(openId);
            if (resultUser.getData() == null) {
                ...
                authUserDto.setUserInfo((User) resultUser.getData());
            }
        }
        ...
        if (!StringUtils.isEmpty(authUserDto.getAuthType()) && authUserDto.getAuthType() == 0) {
            ...
            //判断用户表中是否存在该用户，不存在则进行解密得到用户信息，并进行新增用户
            JsonData resultUser = userService.findByUserName(userName);
            if (resultUser == null || resultUser.getCode() == null || resultUser.getCode() ==1 || resultUser.getData() == null) {
        ...
        return JsonData.buildSuccess(authUserDto);
    }
}
